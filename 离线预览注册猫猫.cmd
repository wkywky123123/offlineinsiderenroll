...
:CHOICE_MENU
cls
title 离线预览注册猫猫 v%scriptver%
set "choice="
echo.
echo 0 - 加入Canary频道（金丝雀）
echo 1 - 加入Dev频道
echo 2 - 加入Beta频道
echo 3 - 加入ReleasePreview频道
echo.
echo 4 - 退出所有已经加入的频道
echo 5 - 不做任何操作并关掉猫猫
echo.
...
:ENROLL_RP
set "Channel=ReleasePreview频道"
set "Fancy=ReleasePreview频道"
set "BRL=8"
set "Content=主线"
set "Ring=External"
set "RID=11"
...
:ENROLL_CAN
set "Channel=Canary频道（金丝雀）"
set "Fancy=Canary频道（金丝雀）"
set "BRL="
set "Content=主线"
set "Ring=External"
set "RID=11"
...
echo "StickyMessage"="{\"Message\":\"主人的设备使用了离线预览注册猫猫进行预览注册\",\"LinkTitle\":\"\",\"LinkUrl\":\"\",\"DynamicXaml\":\"^<StackPanel xmlns=\\\"http://schemas.microsoft.com/winfx/2006/xaml/presentation\\\"^>^<TextBlock Style=\\\"{StaticResource BodyTextBlockStyle }\\\"^>主人的设备已通过 离线预览注册猫猫 v%scriptver% 注册为 Windows预览计划的一部分。 如果主人想更改注册的设置或停止接收 Windows预览版本，请再次用猫猫修改哦~ ^<Hyperlink NavigateUri=\\\"https://github.com/wkywky123123/offlineinsiderenroll\\\" TextDecorations=\\\"None\\\"^>了解更多^</Hyperlink^>^</TextBlock^>^<TextBlock Text=\\\"应用配置\\\" Margin=\\\"0,20,0,10\\\" Style=\\\"{StaticResource SubtitleTextBlockStyle}\\\" /^>^<TextBlock Style=\\\"{StaticResource BodyTextBlockStyle }\\\" Margin=\\\"0,0,0,5\\\"^>^<Run FontFamily=\\\"Segoe MDL2 Assets\\\"^>^&#xECA7;^</Run^> ^<Span FontWeight=\\\"SemiBold\\\"^>%Fancy%^</Span^>^</TextBlock^>^<TextBlock Text=\\\"预览频道: %Channel%\\\" Style=\\\"{StaticResource BodyTextBlockStyle }\\\" /^>^<TextBlock Text=\\\"内容: %Content%\\\" Style=\\\"{StaticResource BodyTextBlockStyle }\\\" /^>^<TextBlock Text=\\\"反馈设置信息\\\" Margin=\\\"0,20,0,10\\\" Style=\\\"{StaticResource SubtitleTextBlockStyle}\\\" /^>^<TextBlock Style=\\\"{StaticResource BodyTextBlockStyle }\\\"^>Windows预览计划要求主人的诊断数据收集设置为 ^<Span FontWeight=\\\"SemiBold\\\"^>开启^</Span^> 状态。 主人可以点击下面的按钮查看或修改当前 ^<Span FontWeight=\\\"SemiBold\\\"^>诊断和反馈^</Span^> 的设置状态。^</TextBlock^>^<Button Command=\\\"{StaticResource ActivateUriCommand}\\\" CommandParameter=\\\"ms-settings:privacy-feedback\\\" Margin=\\\"0,10,0,0\\\"^>^<TextBlock Margin=\\\"5,0,5,0\\\"^>打开诊断和反馈^</TextBlock^>^</Button^>^</StackPanel^>\",\"Severity\":0}"
echo.
)>"%SystemRoot%\oie.reg"
...
:ASK_FOR_REBOOT
set "choice="
echo 需要重启主人的电脑才能完成修改喵
set /p choice="主人需要猫猫重启您的电脑吗？ (y/N) "
